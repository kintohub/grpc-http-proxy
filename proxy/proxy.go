...
type Proxy struct {
	...
	reflector grpcreflection.Reflector
	stub      pstub.Stub
}
...
func (p *Proxy) Connect(ctx context.Context, target *url.URL) error {
	...
	reflectionClient := grpcreflect.NewClient(ctx, rpb.NewServerReflectionClient(p.conn))
	p.reflector = grpcreflection.NewReflector(reflectionClient)
